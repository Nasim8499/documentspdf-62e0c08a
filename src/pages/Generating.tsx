...
const Generating = () => {
  ...
  return (
    <div
      className="min-h-screen w-full flex items-center justify-center md:p-8 relative overflow-hidden"
      style={{ background: "var(--gradient-dark)" }}
    >
      <div
        className="relative w-full md:max-w-[420px] md:h-[860px] md:rounded-[3rem] overflow-hidden flex flex-col min-h-screen md:min-h-0 text-white"
        style={{ background: "var(--gradient-dark)" }}
      >
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8 relative">
          ...
          <h2 className="text-2xl font-bold text-center">
            {progress >= 100 ? "Document ready!" : "AI is crafting your document…"}
          </h2>
          <p className="text-white/60 text-sm text-center mt-2">
            {progress >= 100 ? "Opening preview…" : "Hang tight, magic is happening"}
          </p>
              ...
              <div key={s} className="flex items-center gap-3 bg-white/5 backdrop-blur rounded-2xl px-4 py-3 border border-white/10">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center ${
                    i < currentStep ? "gradient-mint" : i === currentStep ? "gradient-primary" : "bg-white/10"
                  }`}
                >
